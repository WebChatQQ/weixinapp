  ...
  init:function(){
    ...
    app.getInit(function(result){
        ...
        wx.uploadFile({
          ...
          "fid": minisId, "hotshow":0, "categoryId": 0, "pageIndex":1}, // HTTP 请求中其他额外的 form data
          success: function(res){
              var result = JSON.parse(res.data);
  ...
  */
  nextPage: function(e) {
    ...
    wx.uploadFile({
      ...
      formData: {"deviceType":verifyModel.deviceType, "timestamp":verifyModel.timestamp, 
        "uid": unionid, "versionCode":verifyModel.versionCode, "sign":verifyModel.sign,
        "fid": minisId, "hotshow":that.data.hot, "categoryId": that.data.currentTypeId, "pageIndex":pageIndex}, // HTTP 请求中其他额外的 form data
      success: function(res){
          console.log("下拉刷新成功");
          var result = JSON.parse(res.data);
  ...
  */
  ...
  typeChange: function(typeId) {
      ...
      wx.uploadFile({
        ...
        formData: {"deviceType":verifyModel.deviceType, "timestamp":verifyModel.timestamp, 
        "uid": unionid, "versionCode":verifyModel.versionCode, "sign":verifyModel.sign,
        "fid": minisId, "hotshow":0, "categoryId": typeId, "pageIndex":1}, // HTTP 请求中其他额外的 form data
        success: function(res){
            var result = JSON.parse(res.data);
  ...
  */
  replyComment:function() {
      ...
      wx.uploadFile({
        ...
        formData: {"deviceType":verifyModel.deviceType, "timestamp":verifyModel.timestamp, 
        "uid": unionid, "versionCode":verifyModel.versionCode, "sign":verifyModel.sign,
        "artId":showRecomment.id, "toUserId":showRecomment.toUserId, "commontId":showRecomment.commontId, "comment":content, "images":imgs}, // HTTP 请求中其他额外的 form data
        success: function(res){
            var result = JSON.parse(res.data);
            if (result.result == true) { // 发帖成功
                wx.request({
